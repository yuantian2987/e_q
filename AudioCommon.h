...
static inline audio_coef_sample_acc_t mul_coef_sample(audio_coef_t x, audio_sample_t y) {
    return ((audio_coef_sample_acc_t) (x)) * y;
}

// Multiply and accumulate sample by a coefficient to return an accumulator.
static inline audio_coef_sample_acc_t mac_coef_sample(audio_coef_t x, audio_sample_t y, audio_coef_sample_acc_t acc) {
    return acc + ((audio_coef_sample_acc_t) (x)) * y;
}

// Convert a sample-coefficient accumulator to a sample.
static inline audio_sample_t coef_sample_acc_to_sample(audio_coef_sample_acc_t acc) {
    if (acc < 0) {
...
static inline audio_sample_t s15_to_audio_sample_t(int16_t s15) {
    return ((audio_sample_t)(s15)) << 9;
}

// Convert a audio_sample_t sample to S15 (no clipping)
static inline int16_t audio_sample_t_to_s15(audio_sample_t sample) {
    return (int16_t)((sample + (1 << 8)) >> 9);
}

// Convert a audio_sample_t sample to S15 (with clipping)
static  inline int16_t audio_sample_t_to_s15_clip(audio_sample_t sample) {
