...
int __builtin_clz(unsigned int type)///added by wangwp, to compile succeed
{
    int num = 0;
...
int32_t Effects_log2(uint32_t x) {
    ...
    return (int32_t)(((int64_t)exp << 15) + log + (((int64_t)(x - segStart) * (logEnd - log)) >> (exp - 6)));
}
...
uint32_t Effects_exp2(int32_t x) {
    ...
    return (uint32_t)((((uint64_t)exp << 9) + (uint64_t)(expEnd - exp) * x) >> (31 - i));
}
